...
export default {
  ...
  methods: {
    ...
    drawDirectionText(position, direction, fromPoint, toPoint, isFromA) {
      ...
      // 计算文本位置，根据箭头方向决定文本应该出现在哪一侧
      const normalX = -direction.y;  // 垂直方向
      const normalY = direction.x;   // 垂直方向
      
      // 设置文本位置在箭头侧边，而不是正上方/正下方
      let offsetX = 0, offsetY = 0;
      
      // 如果箭头方向比较水平，文本放在上方或下方
      if (Math.abs(direction.x) > Math.abs(direction.y)) {
        // 水平箭头，文本放在上方或下方
        offsetY = direction.x > 0 ? -25 : 25; // 从左到右箭头，文本在上方；从右到左箭头，文本在下方
      } else {
        // 垂直箭头，文本放在左侧或右侧
        offsetX = direction.y > 0 ? 25 : -25; // 从上到下箭头，文本在右侧；从下到上箭头，文本在左侧
      }
      ...
      this.ctx.fillRect(
        finalTextX - bgWidth / 2, 
        finalTextY - bgHeight / 2, 
        bgWidth, 
        bgHeight
      );
      ...
      this.ctx.strokeText(text, finalTextX, finalTextY);
      
      this.ctx.fillStyle = '#FFFFFF';
      this.ctx.fillText(text, finalTextX, finalTextY);
