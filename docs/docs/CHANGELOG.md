...
- ​新安装不再支持 Ubuntu 20.04。​​

### 错误修复

- 修复：在 Apache 2.4.64 更新后，所有 Web 和邮件域名均出现 421 错误 (#5058)。
- 修复：将默认的 SOA 重试值（Retry Value）设为 1800 以符合 DENIC 要求 (#5030)。
- 修复：更改 Web 域名时的域名别名替换逻辑（修复 #5015）(#5041)。
- 修复：IPv4 CIDR 验证问题 (#5044)。
- 更新 magento.tpl / magento.stpl 模板以支持健康检查（healthcheck）(#5036)。
- 新增：在仪表盘上显示用户和带宽配额。
- 在对备份文件列表执行保留检查前先进行排序（修复 #5017）(#5018)。
- 改进 Spamhaus DQS 查询的日志记录，避免暴露查询密钥 (#5011)。
- 将 Roundcube 升级至 1.6.11 版本。
- 在 Debian 安装程序中移除了 apache2-suexec-pristine 软件包。
- 修复 (#4979)：修复了域名重定向未被暂停的问题 (#4991)。
- 确保 hestiaweb 用户的 crontab 文件末尾有换行符 (#4992)。
- 允许添加 SMTP 中继用户名时使用斜杠“/”（修复 #4973）(#4974)。
- 修复：防止空的用户变量影响多个脚本（修复 #4926）(#4928)。
- 修复管理面板（Panel）为 hestiaweb 用户设置定时任务（Cronjobs）的问题 (#4891)。
- 修复 Ubuntu 上缺少的 proftpd-mod-crypto 依赖项 (#4895)。
...
- 从[packages.sury.org](https://forum.hestiacp.com/t/apt-upgrade-failed-gpg-error-packages-sury-org) 更新了 PHP 的 APT 存储库密钥
...
- 提高了整个控制面板 UI 中工具提示的准确性和可靠性：
  ...
  - 修复了导致工具提示无法显示的错误标签。
  - 引入了工具提示来计数器“用户”、“包”和“统计信息”页面上的项目，以帮助更好地区分统计信息。
...
- 改进了对“v-add-remote-dns-host”命令 API 密钥使用的支持 (#1265)
...
- 更正了使用保加利亚语或希腊语时，由于字符串长度原因，工具栏在邮件和防火墙页面上不合适的问题。
- 改进了西班牙语翻译（感谢 **@Wibol**）
...
- 修复了使用非英语语言环境时用户界面元素会重叠或显示在错误位置的问题。
...
- v-restore-user 仅适用于存储在 /backup 挂载点中的备份存档 (#641)
...
- 修复了远程备份主机名会拒绝没有反向 DNS（PTR 记录）的 IP 地址的问题。 (#569)
